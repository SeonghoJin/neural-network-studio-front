...
파이썬 코드 변환은 잘 만들어져도 실행이 안될 경우가 있습니다. 실행 되기 위해서는 그래프의 블록 Config값들이 제대로 설정되어 있어야하고 이를 제대로 설정하기 위해서는 인공지능과 관련된 지식이 필요합니다. 제대로 설정만 된다면 위의 페이지와 같이 학습이 진행되는 모습을 실시간으로 그래프로 보실 수 있습니다.
